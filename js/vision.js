...
var Vision = (function () {
  ...
  var bestLabel = 0, blobShare = 0, blobCount = 0;
  ...
  function pickBlob() {
    label.fill(0);
    var cur = 0, i, p, x, y, sp, area, sx, sy;
    var best = 0, bestScore = -1, bestArea = 0, total = 0, big = 0;

    for (i = 0; i < NPX; i++) {
      if (!mask[i] || label[i]) continue;
      cur++;
      sp = 0; area = 0; sx = 0; sy = 0;
      ...
      /* 큰 덩어리를 고르되, 조금 전까지 따라가던 자리와 가까우면 더 쳐준다
         (비슷한 크기의 물체 사이에서 왔다 갔다 하지 않도록) */
      var cx = sx / area, cy = sy / area, score = area;
      if (prevCx >= 0) {
        var d = Math.hypot(cx - prevCx, cy - prevCy);
        if (d < W * 0.12) score *= 1.6;
      }
      if (score > bestScore) { bestScore = score; best = cur; bestArea = area;
                               prevCandX = cx; prevCandY = cy; }
    }
